...
// ==============================
// REVIEW LIKE BUTTON — global handler
// ==============================
(function () {
    var LIKED_KEY = 'neovala_liked_reviews';
    ...
    function isLiked(id) {
        return getLiked().indexOf(String(id)) !== -1;
    }
    ...
    function applyLikedState(btn, liked) {
        if (liked) {
            btn.classList.add('text-[#674c1d]');
            btn.classList.remove('text-stone-400');
            btn.setAttribute('title', 'Sudah disukai');
            btn.querySelector('i').classList.add('fas');
            btn.querySelector('i').classList.remove('far');
        } else {
            btn.classList.remove('text-[#674c1d]');
            btn.classList.add('text-stone-400');
            btn.setAttribute('title', 'Suka');
            btn.querySelector('i').classList.add('fas');
        }
    }
    ...
    function initLikeButtons() {
        document.querySelectorAll('.review-like-btn').forEach(function (btn) {
            ...
            if (!id) return;
            applyLikedState(btn, isLiked(id));
    ...
    document.addEventListener('DOMContentLoaded', function () {
        initLikeButtons();
        // Re-init after dynamic renders (mutation observer for AJAX-rendered cards)
        var observer = new MutationObserver(function () {
            initLikeButtons();
        });
        var containers = [
            document.getElementById('reviews-list'),
            document.getElementById('reviews-detail-list'),
            document.getElementById('reviews-widget-list'),
            document.getElementById('reviews-page-list'),
        ];
