            ...
            listEl.addEventListener('click', function(e) {
                ...
                if (btn) {
                    ...
                    allBtns.forEach(function(b) {
                        var src = b.getAttribute('data-src'); var type = b.getAttribute('data-type') || 'image';
                        if (src) { if (b === btn) clickedIdx = items.length; items.push({ src: src, type: type }); }
                    });
                    if (items.length && typeof window.openMediaGallery === 'function') {
                        window.openMediaGallery(items, clickedIdx);
                    } else if (items.length) { openWidgetMedia(items[clickedIdx].src, items[clickedIdx].type); }
                    return;
                }
