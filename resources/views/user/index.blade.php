...
<section class="apartment-section" id="apartment-section">
    ...
    <div class="apartment-container">
        ...
        <!-- Apartment Cards -->
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_0020 (Copy).webp') }}" name="TRANSPARK JUANDA"
            :route="route('discoverTPJ')" />
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_9497.webp') }}" name="TRANSPARK CIBUBUR"
            :route="route('discoverTPC')" />
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_0117 (Copy).webp') }}"
            name="GRAND KAMALA LAGOON" :route="route('discoverGKL')" />
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_1073.webp') }}" name="PATRALAND URBANO"
            :route="route('discoverPLU')" />
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_3976.webp') }}" name="GATEWAY CICADAS"
            :route="route('discoverGWC')" />
        <x-apartment-card image="{{ asset('images/images/home pages/IMG_0362.webp') }}" name="PODOMORO GOLF VIEW"
            :route="route('discoverPGV')" />
        <x-apartment-card image="{{ asset('images/images/discover-GPC/IMG_0646.webp') }}" name="GREEN PRAMUKA CITY"
            :route="route('discoverGPC')" />
        <x-apartment-card image="{{ asset('images/images/discover-BSC/IMG_1882.webp') }}" name="BASSURA CITY"
            :route="route('discoverBSC')" />
        <x-apartment-card image="{{ asset('images/images/discover-SPL/IMG_9338.webp') }}" name="SPRINGLAKE SUMMARECON"
        ...
        <div class="guest-service-content">
            ...
            <div class="guest-service-right">
                ...
                <p class="guest-service-text">
                    Layanan pelanggan 24/7 untuk membantu kebutuhan Anda. Tim kami siap membantu dengan profesional dan
                    ramah.
                </p>
            </div>
        </div>
    ...
    <div class="titip-kunci-container">
        ...
        <div class="titip-kunci-content">
            ...
            <div class="content-wrapper">
                ...
                <img src="{{ asset('images/logo/handshake-icon.webp') }}" alt="Handshake Icon" class="handshake-icon">
            </div>
        </div>
    </div>
</section>
    ...
    function renderCard(r) {
        ...
        if (r.replies && r.replies.length) {
            const replyBlocks = r.replies.map(function(rep) {
                return '<p class="text-[11px] text-[#674c1d] font-medium">' + (rep.admin_name || 'Admin') +
                    '</p><p class="text-[11px] text-stone-600 leading-tight">' + (rep.content || '') +
                    '</p>';
            }).join('');
...
(function() {
    ...
    function closeReplyOnClickOutside(ev) {
        if (openReplyDropdown && openReplyBtn && !openReplyDropdown.contains(ev.target) && !openReplyBtn.contains(ev
                .target)) {
            closeReplyDropdown();
        }
    }
    ...
    if (reviewsList && sliderOuter) {
        reviewsList.addEventListener('click', function(e) {
            ...
            dropdown.style.cssText = 'position:absolute;left:' + leftPx + 'px;top:' + topPx + 'px;width:' +
                Math.max(btnRect.width, 200) + 'px;min-width:200px;z-index:9999;';
