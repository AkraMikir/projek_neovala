                ...
                <div class="comment-footer">
                    ...
                    <div class="comment-actions">
                        @if ($komen->status === 'pending')
                            <form action="{{ route('admin.dashboard1.gpc.applyComment', $komen->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="apply-btn">Apply</button>
                            </form>
                        @else
                            <form action="{{ route('admin.dashboard1.gpc.unapplyComment', $komen->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('PATCH')
                        ...
                        <form action="{{ route('admin.dashboard1.gpc.deleteComment', $komen->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
